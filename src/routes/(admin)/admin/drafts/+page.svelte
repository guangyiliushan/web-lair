				...
				<div class="divide-y divide-border">
					{#each filteredDrafts as draft (draft.id)}
						...
						>
							...
							<input
								type="checkbox"
								class="mt-1 size-4 rounded border-border bg-transparent"
								onclick={(e) => e.stopPropagation()}
							/>
							<div class="flex min-w-0 items-start gap-3">
								<span
									class="mt-0.5 inline-flex shrink-0 items-center gap-1 rounded-xs border px-2 py-1 text-xs font-medium {cfg.class}"
								>
	...
	<MasterDetail.Pane side="detail" class="bg-surface-card hidden lg:flex">
		{#if selectedDraft}
			...
			<div class="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)]">
				<!-- 版本列表 -->
				<aside class="min-h-0 border-b lg:border-r lg:border-b-0">
					...
					<div class="min-h-0 overflow-auto lg:max-h-[calc(100%-2.5rem)]">
						{#each sd.versions as v (v.id)}
							<article
								class="group flex w-full items-center gap-3 border-b px-4 py-3 transition-colors hover:bg-muted/50"
							>
								<div class="min-w-0 flex-1">
				...
				<div class="flex min-h-0 flex-col bg-muted/30">
					<div
						class="flex h-10 shrink-0 items-center justify-between border-b px-4 text-xs text-muted-foreground"
					>
					...
					<div class="min-h-0 flex-1 overflow-auto p-4">
						<div class="flex items-center justify-between gap-3">
							...
							<span class="text-xs text-muted-foreground tabular-nums">v{sd.version}</span>
						</div>
